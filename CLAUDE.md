...
- Skill complementario: `.claude/skills/context-frugality/SKILL.md` — frugalidad de contexto
  en loops de implementación (`/sdd-implement`, `/sdd-task`, `/sdd-fix`). No decide qué archivos
  son del dominio (eso lo sigue haciendo `graph/domain.yaml`) ni baja gates de calidad; decide
  cuánto de cada archivo y de cada output de comando hace falta cargar.

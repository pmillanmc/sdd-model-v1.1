...
| `tokens_estimados` | El bloque DX_MET_006 tiene dos formatos según la variante que aplicó (ver `sdd-metrics.md` Paso 0.5) — no asumas cuál es sin mirar el archivo: **Variante A** (Claude Code, `source: ccusage`) usa `**TOTAL**: [N]`, tokens reales; **Variante B** (bytes÷4) usa `**TOTAL INPUT estimado**`, una estimación. Tomá el último bloque `### 📊 Reporte de Esfuerzo SDD` del archivo y usá el campo que corresponda a su variante. Si ese último bloque tiene `feature_total: true`, es el cierre definitivo de la feature — marcalo como tal en la tabla (ej. con un ✓ aparte) en vez de mezclarlo sin distinción con cortes parciales de otras features. |
...
| feature_id | Iteraciones | Tokens | Cierre | Rework Ratio | Cobertura inicial | Autocorr. | Consultas |
|---|---|---|---|---|---|---|---|
| [valor] | [N] | [N] | [✅ si `feature_total: true` en el último reporte / ⏳ parcial si no] | [ratio] | [%] | [N] | [N] |

...
3. Registrá la sesión para atribución de tokens: obtené el session_id actual con
   `!echo $CLAUDE_CODE_SESSION_ID` y, si no está vacío, agregalo como línea nueva
   (append — nunca sobrescribir) a `metrics/[feature_id].sessions`, creándolo si no
   existe. Cada `/sdd-task` puede ser una sesión distinta (reparto entre devs, o
   retomar una task bloqueada en otro momento) y todas cuentan para el costo total de
   la feature que `/sdd-metrics` suma al cerrar con `/sdd-review`.
